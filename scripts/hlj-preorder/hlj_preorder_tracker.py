#!/usr/bin/env python3
"""
scripts/hlj-preorder/hlj_preorder_tracker.py
============================================
...
Depends on: scripts/app/hlj_helpers.py (shared with hlj-lowstock).
...
MAX_PAGES        = 20
PAGE_CONCURRENCY = 3   # listing pages walked in parallel
OUTPUT_DIR       = _HERE
PREORDER_FIELDS = [
...
async def _scrape_card(card, detail_page, rates, idx):
    """Extract one preorder card; return item dict or None if filtered out."""
    ...
    try:
        ...
        try:
            el = await detail_page.query_selector(
                "//li[starts-with(normalize-space(.), 'Release Date:')]"
            )
            if el:
        ...
        try:
            el = await detail_page.query_selector(
                "//li[starts-with(normalize-space(.), 'Cancellation Deadline:')]"
            )
            if el:
                raw = (await el.inner_text() or "").strip()
...
async def _scrape_page(context, page_num, max_pages, rates, sem, counter):
    ...
    async with sem:
        ...
        try:
            ...
            try:
                ...
                print(f"  WARN page {page_num}: No prices — continuing")
            try:
            ...
            for card in cards:
                ...
                idx = counter[0]
                try:
                    item = await _scrape_card(card, detail_page, rates, idx)
                    if item:
                        items.append(item)
...
async def scrape_preorders(max_pages: int = MAX_PAGES) -> list:
    ...
    async with async_playwright() as pw:
        ...
        counter = [0]
        tasks = [_scrape_page(context, p, max_pages, rates, sem, counter)
    ...
    preorders = [item for _, items in page_buckets for item in items]
