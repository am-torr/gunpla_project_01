...
class HobbyLinkJapanScraper(BaseScraper):
    ...
    async def scrape(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            async with async_playwright() as p:
                ...
                print(f"📊 Fetching details for {limit} products...")
            ...
            print(f"❌ Scrape failed for {self.store_name}: {e}")
            import traceback
            traceback.print_exc()
            raise
    ...
    async def scrape_product_details(self, page, product_url: str) -> Dict:
        ...
        try:
            ...
            print(f"  ✅ Code={details['code']}, JAN={details['jan_code']}, Mfr={details['manufacturer']}")
    ...
    def extract_jan_code(self, soup: BeautifulSoup) -> str:
        ...
        try:
            ...
            print(f"    ⚠️ JAN extraction failed: {e}")
        return None
    ...
    def extract_release_date(self, soup: BeautifulSoup) -> str:
        ...
        try:
            ...
            print(f"    ⚠️ Release date extraction failed: {e}")
        return None
    ...
    def extract_series(self, soup: BeautifulSoup) -> List[str]:
        ...
        try:
            ...
            print(f"    ⚠️ Series extraction failed: {e}")
        return []
    ...
    def extract_includes(self, soup: BeautifulSoup) -> List[str]:
        ...
        try:
            ...
            print(f"    ⚠️ Includes extraction failed: {e}")
        return []
    ...
    def extract_manufacturer(self, soup: BeautifulSoup) -> str:
        ...
        try:
            ...
            print(f"    ⚠️ Manufacturer extraction failed: {e}")
        return None
    ...
    def extract_item_type(self, soup: BeautifulSoup) -> str:
        ...
        try:
            ...
            print(f"    ⚠️ Item Type extraction failed: {e}")
        return None
    ...
    def extract_cancellation_deadline(self, soup: BeautifulSoup) -> str:
        ...
        try:
            ...
            print(f"    ⚠️ Cancellation deadline extraction failed: {e}")
        return None

    def parse_product(self, product_element) -> Dict:
        """Parse individual product from listing page"""

        name_elem = product_element.select_one(HLJSelectors.PRODUCT_NAME)  # UPDATED
        name_text = name_elem.text.strip() if name_elem else "Unknown"
        product_url = (
            self.base_url + name_elem["href"]
            if name_elem and name_elem.get("href")
            else self.gunpla_url
        )

        price_elem = product_element.select_one(HLJSelectors.PRODUCT_PRICE)  # UPDATED
        ...
        img_elem = product_element.select_one(HLJSelectors.PRODUCT_IMAGE)  # UPDATED
        image_url = None
        if img_elem and img_elem.get('src'):
            img_src = img_elem['src']
            image_url = f"https:{img_src}" if img_src.startswith('//') else img_src

        stock_detail_elem = product_element.select_one(HLJSelectors.STOCK_STATUS)  # UPDATED
        stock_status = stock_detail_elem.text.strip() if stock_detail_elem else "Unknown"

        on_sale_elem = product_element.select_one(HLJSelectors.ON_SALE_FLAG)  # UPDATED
        is_on_sale = on_sale_elem and 'On Sale' in on_sale_elem.text
